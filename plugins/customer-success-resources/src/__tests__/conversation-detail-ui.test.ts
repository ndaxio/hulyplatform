...
describe('conversation detail UI security contract', () => {
  ...
  it('reactively subscribes to issue truth and the issue-bound internal live-session projection', () => {
    ...
    expect(detailSource).toContain('resolveTakeoverChromeState({ ...issue, assignee: cachedAssignee }, liveSessionState)')
    expect(detailSource).toContain('{ ...target, assignee: canonicalAssignee }')
  ...
  it('keeps takeover chrome request-scoped and confirms success only through reconciled issue status', () => {
    ...
    expect(takeoverSource).toContain('if (!assignLead.canSubmit || event.detail == null) return')
    ...
    expect(takeoverSource).toContain('aria-disabled={!assignLead.canSubmit}')
    expect(takeoverSource).toContain('aria-busy={assignLead.busy}')
